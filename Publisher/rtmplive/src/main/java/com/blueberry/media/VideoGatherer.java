...
public class VideoGatherer {
    ...
    public Params initCamera(Activity act, SurfaceHolder holder) {
        ...
        //setPreviewCallbackWithBuffer、setPreviewCallback、setOneShotPreviewCallback被调用的使用，一定得调用addCallbackBuffer
    ...
    private void initWorkThread(final Activity activity) {
            ...
            public void run() {
                while (loop && !Thread.interrupted()) {
                    try {
                        ...
                        // 由于Android硬编编码器支持I420(COLOR_FormatYUV420Planar)和NV12(COLOR_FormatYUV420SemiPlanar)，
                        // 因此可以将N21的图片转为I420或者NV12，然后交给硬编编码器。
                        ...
                        if (mCallback != null) {
                            if(Utils.isScreenOriatationPortrait(activity)){
                                byte[] result = new byte[dstByte.length];
                                Yuv420Util.rotateYUV420Deg90(result, dstByte, previewSize.width, previewSize.height);
                                mCallback.onReceive(result, colorFormat);
                            }else{
     ...
     * @param isFront
     */
    public void setCarmerOrientation(boolean isFront){
