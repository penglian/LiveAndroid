 ...
 * 在编解码器的生命周期内有三种理论状态：停止态-Stopped、执行态-Executing、释放态-Released，停止状态（Stopped）
 * 包括了三种子状态：未初始化（Uninitialized）、配置（Configured）、错误（Error）。执行状态（Executing）
 * 在概念上会经历三种子状态：刷新（Flushed）、运行（Running）、流结束（End-of-Stream
 */
...
public class MediaEncoder {
    ...
    private void encodeVideoData(byte[] dstByte) {
        ...
        //返回一个填充了有效数据的input buffer的索引，如果没有可用的buffer则返回-1.当timeoutUs==0时，该方法立即返回；
        // 当timeoutUs<0时，无限期地等待一个可用的input buffer;当timeoutUs>0时，至多等待timeoutUs微妙。
        ...
        if (inputBufferId >= 0) {
            ...
            //给指定索引的input buffer填充数据后，将其提交给codec组件。一旦一个input buffer在codec中排队，
            // 它就不再可用直到通过getInputBuffer(int)重新获取，getInputBuffer(int)是对dequeueInputbuffer
            // (long)的返回值或onInputBufferAvailable(MediaCodec, int)回调的响应。
